        ...
        <div className="flex-1 space-y-6 px-5 py-6 sm:px-8">
          <section className="fade-up rounded-[28px] border border-[#efe7d8] bg-[linear-gradient(135deg,#fffef9_0%,#fff9e8_50%,#fff5d5_100%)] p-6 shadow-[0_12px_40px_rgba(24,24,24,0.05)] sm:p-8">
            <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <button type="button" onClick={() => !isPendingApproval && setComposerOpen(true)} disabled={isPendingApproval} className="primary-button group flex items-center gap-2 self-start whitespace-nowrap px-8 py-4 text-base font-semibold transition-all duration-200 hover:-translate-y-1 hover:shadow-2xl active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60"><span className="text-xl transition-transform duration-300 group-hover:rotate-90">+</span><span>{isPendingApproval ? "Activation required" : "New Post"}</span></button>
            </div>
          ...
          <section className="fade-up fade-up-2 grid gap-6 2xl:grid-cols-[1.5fr_0.85fr]">
            ...
            <div className="panel flex flex-col p-5 sm:p-6">
              ...
              <button type="button" onClick={() => !isPendingApproval && setComposerOpen(true)} disabled={isPendingApproval} className="field-input group mb-4 min-h-[100px] cursor-text text-left text-base text-ink-500 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-60"><span className="transition-colors duration-150 group-hover:text-brand-600">{isPendingApproval ? "Posting unlocks after account activation." : "What's on your mind?"}</span></button>
              ...
              <div className="mt-auto rounded-[20px] border border-[#252030] bg-gradient-to-br from-[#0d0b14] to-[#100e1a] p-4"><p className="text-sm font-medium leading-6 text-ink-700">{stats.queuedPosts > 0 ? `${stats.queuedPosts} post${stats.queuedPosts > 1 ? "s" : ""} queued for delivery` : "Schedule your first post today"}</p><p className="mt-1 text-xs text-ink-500">{stats.connectedPlatforms} platform{stats.connectedPlatforms !== 1 ? "s" : ""} ready</p><button type="button" onClick={() => !isPendingApproval && setComposerOpen(true)} disabled={isPendingApproval} className="primary-button mt-4 w-full py-3.5 text-base font-semibold transition-all duration-200 hover:-translate-y-1 hover:shadow-xl active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60">{isPendingApproval ? "Activation required" : "Open Composer"}</button></div>
            </div>
          </section>
