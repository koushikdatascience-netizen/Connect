...
export default function PostsStudio() {
  ...
  return (
    <>
      <main className="min-h-[calc(100vh-2.5rem)] px-4 py-4 pb-24 sm:px-5 sm:pb-28 lg:pb-6 lg:px-6">
          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_320px]">
            <div className="space-y-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <button type="button" onClick={() => !isPendingApproval && router.push("/create-post")} disabled={isPendingApproval} className="primary-button px-5 py-3 text-sm disabled:cursor-not-allowed disabled:opacity-60">{isPendingApproval ? "Activation required" : "Create Post"}</button>
              </div>
                      ...
                      return (
                        <button key={post.id} type="button" onClick={() => setSelectedPostId(post.id)} className={`flex w-full items-center gap-3 rounded-[20px] border p-3 text-left transition ${selectedPostId === post.id ? "border-[#e1ca8b] bg-[#fff9e9]" : "border-[#eee4d6] bg-[#fffef9] hover:border-[#e2d4b1]"}`}>
                          ...
                          <div className="flex gap-2">
                            ...
                            >
                              {actionDisabled ? "Activation required" : "Edit"}
                            </button>
                            <button
                              type="button"
                              onClick={(event) => {
